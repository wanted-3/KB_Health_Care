  ...
  if (items.healthScoreList && items.healthScoreList) {
    items.healthScoreList
      ...
      .forEach((item: IHealthScoreList) =>
        data.push({ x: dayjs(item.SUBMIT_DATE).format('YYYY'), y: Number(item.SCORE), label: Number(item.SCORE) })
      )
    ...
    return (
      <div>
        {(difference > 0 && (
          <p>
            3건강 점수는 총점이 지난해 보다 <mark>{difference}점 높아요</mark>
          </p>
        )) ||
          (difference < 0 && (
            <p>
              건강 점수는 총점이 지난해 보다 <mark>{-1 * difference}점 낮아요</mark>
            </p>
          )) ||
